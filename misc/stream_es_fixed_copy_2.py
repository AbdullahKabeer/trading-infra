...
def get_or_create_session():
    ...
    if not _rth_session_ready and is_rth():
        ...
        if global_bot.pos:
            ...
            global_bot.pos=None  # force flat at RTH open
...
class VWAPBot:
    ...
    async def close_position(self, price, reason, bar_idx):
        ...
        print(f"\n  [BOT] CLOSING {self.pos['dir']} @ {price:.2f} ({reason}) | PnL: ${pnl:.2f}")
        act = "SELL" if self.pos["dir"] == "LONG" else "BUY"
        success = await self.place_order(act, CONTRACTS)
        if success or DRY_RUN:  # BUG9: only commit PnL if order actually succeeded
            self.total_pnl += pnl
            self.daily_pnl += pnl
    ...
    async def check_logic(self, price, vwap, vpoc, std, vwap_z, bar_idx, rth, stable=True):
        ...
        if self.pos:
            ...
            if self.pos["dir"] == "LONG":
                ...
                if self.gutter_loss and self.daily_pnl > -GUTTER_DD:  # BUG4: guard
            ...
            await self.refresh_token_if_needed()  # BUG6: check token age
            ...
            if not stable: return  # FIX: skip entries on trend days (VPOC migrating)
            z_abs = abs(target_z)
            if z_abs >= Z_THRESH:
...
async def on_trade(args,rth=True):
    ...
    async with BOT_LOCK:  # BUG1: protect bot state with async lock
        await global_bot.check_logic(snap_price, snap_vwap, snap_vpoc, snap_std, cz, bar_idx, rth, stable)
