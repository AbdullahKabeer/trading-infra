...
GUTTER_GOAL = 1500
GUTTER_DD = 1900
COMMISSION_RT = 2.80  # round-turn commission per contract (NFA + clearing)
...
class Session:
    ...
    def add_trade(self,price,vol,side_str,now_utc):
        ...
        if self.cur_bar is None or self.cur_bar["ts"]!=bk_str:
            if self.cur_bar:
                ...
                self.vpoc_history.append(self.vpoc)  # FIX: track VPOC on bar completion
    ...
    def add_quote(self,bid_v,ask_v,now_utc):
        ...
        if len(self.quotes)>200:del self.quotes[:-200]
    ...
    def to_dict(self):
        ...
        return {"date":self.date,"bid":self.bid,"ask":self.ask,"last":self.last,
            ...
            "bars":ab,"current_bar":cb,"tape":self.tape[-200:],"quotes":self.quotes[-200:]}
    ...
    def load(date_str):
        ...
        # BUG3: rebuild vpoc_history from bars
...
def get_or_create_session():
    ...
    if not _rth_session_ready and is_rth():
        ...
        if global_bot.pos:
            print(f"\n  [WARNING] Open position carried into new day — closing")
            global_bot.pos=None
...
class VWAPBot:
    ...
    async def close_position(self, price, reason, bar_idx, bid=0, ask=0):
        # Use bid for SELL fills, ask for BUY fills (realistic market order execution)
        if self.pos["dir"] == "LONG":
            fill_price = bid if bid > 0 else price  # closing LONG = SELL @ bid
            ticks = (fill_price - self.pos["ep"]) / TICK_SIZE
        else:
            fill_price = ask if ask > 0 else price  # closing SHORT = BUY @ ask
            ticks = (self.pos["ep"] - fill_price) / TICK_SIZE
        pnl = ticks * TICK_VALUE * CONTRACTS
        comm = COMMISSION_RT * CONTRACTS
        pnl -= comm  # deduct round-turn commission
        print(f"\n  [BOT] CLOSING {self.pos['dir']} @ {fill_price:.2f} ({reason}) | PnL: ${pnl:.2f} (comm: -${comm:.2f})")
        act = "SELL" if self.pos["dir"] == "LONG" else "BUY"
        success = await self.place_order(act, CONTRACTS)
        if success or DRY_RUN:
            self.total_pnl += pnl
            self.daily_pnl += pnl
            self.trade_history.append({"action": "EXIT", "dir": self.pos["dir"], "price": fill_price, "pnl": pnl, "bar_idx": bar_idx})
            self.pos = None
            print(f"  [BOT] Total: ${self.total_pnl:.2f}")
        else:
            print(f"  [BOT] ⚠ ORDER FAILED — position still open, will retry next tick")

    async def check_logic(self, price, vwap, vpoc, std, vwap_z, bar_idx, rth, stable=True, bid=0, ask=0):
        if not self.ready or not vwap or not std or bar_idx < 15: return
        if not rth:
            if self.pos: await self.close_position(price, "END OF RTH", bar_idx, bid, ask)
            return
        ...
        if self.pos:
            ...
            if self.pos["dir"] == "LONG":
                if self.gutter_win and self.daily_pnl < GUTTER_GOAL:
                    ...
                    if price >= gut_tp: await self.close_position(gut_tp, "GUTTER WIN", bar_idx, bid, ask); return
                if self.gutter_loss and self.daily_pnl > -GUTTER_DD:
                    remaining = (self.daily_pnl + GUTTER_DD) / (CONTRACTS*TICK_VALUE)
                    if remaining > 0:
                        gut_sl = self.pos["ep"] - remaining * TICK_SIZE
                        if price <= gut_sl: await self.close_position(gut_sl, "GUTTER LOSS", bar_idx, bid, ask); return
                
                if price > self.pos["bp"]: self.pos["bp"] = price

                ur = (self.pos["bp"] - self.pos["ep"]) / TICK_SIZE
                cur = (price - self.pos["ep"]) / TICK_SIZE
                if price <= self.pos["sl"]: await self.close_position(price, "STOP LOSS", bar_idx, bid, ask); return
                if target and price >= target and cur >= EXIT_MIN_TICKS: await self.close_position(price, "TARGET", bar_idx, bid, ask); return
                ...
                if bt > TIME_STOP_MINS and cur > -4: await self.close_position(price, "TIME STOP", bar_idx, bid, ask); return
            else:
                if self.gutter_win and self.daily_pnl < GUTTER_GOAL:
                    gut_tp = self.pos["ep"] - ((GUTTER_GOAL - self.daily_pnl) / (CONTRACTS*TICK_VALUE)) * TICK_SIZE
                    if price <= gut_tp: await self.close_position(gut_tp, "GUTTER WIN", bar_idx, bid, ask); return
            ...
            if z_abs >= Z_THRESH:
                ...
                if target_z < 0:
                    fill = ask if ask > 0 else price  # BUY fills at ask
                    sl = round((fill - sd) / TICK_SIZE) * TICK_SIZE
                    tp = target + 4 * TICK_SIZE
                    print(f"\n  [BOT] ENTRY LONG @ {fill:.2f} (ask) | Tgt: {tp:.2f} | Z={abs(target_z):.1f}")
                    if await self.place_order("BUY", CONTRACTS):
                        self.pos = {"dir": "LONG", "ep": fill, "sl": sl, "tp": tp, "bp": fill, "bar_idx": bar_idx}
                        self.trades_today += 1
                        self.trade_history.append({"action": "ENTER", "dir": "LONG", "price": fill, "bar_idx": bar_idx})
                elif target_z > 0:
                    fill = bid if bid > 0 else price  # SELL fills at bid
                    sl = round((fill + sd) / TICK_SIZE) * TICK_SIZE
                    tp = target - 4 * TICK_SIZE
                    print(f"\n  [BOT] ENTRY SHORT @ {fill:.2f} (bid) | Tgt: {tp:.2f} | Z={abs(target_z):.1f}")
                    if await self.place_order("SELL", CONTRACTS):
                        self.pos = {"dir": "SHORT", "ep": fill, "sl": sl, "tp": tp, "bp": fill, "bar_idx": bar_idx}
                        self.trades_today += 1
                        self.trade_history.append({"action": "ENTER", "dir": "SHORT", "price": fill, "bar_idx": bar_idx})
...
def run_backtest(bars, gutter_win=False, gutter_loss=False):
    ...
    peak_pnl = 0; max_dd = 0
    daily_pnl = 0
    vpoc_hist = []  # FIX: track VPOC for stability filter
    ...
    for i, b in enumerate(bars):
        ...
        vpoc_hist.append(vpoc)  # FIX: track per bar
        
        # Stability check: VPOC migration speed over last 20 bars
        stable = True
        if len(vpoc_hist) >= 20:
            recent = vpoc_hist[-20:]
            valid = [p for p in recent if p and p > 0]
        ...
        if pos:
            ...
            if pos["dir"] == "LONG":
                ...
                if gutter_loss and daily_pnl > -GUTTER_DD:  # BUG4/10: guard
                ...
                if h > pos["bp"]: pos["bp"] = h  # FIX: use high, not close
                ...
                if l < pos["bp"]: pos["bp"] = l  # FIX: use low, not close
            ...
            if not stable: continue  # FIX: skip entries on trending sessions
...
async def stream(token):
    ...
    async for ws in websockets.connect(ws_url,additional_headers={"Authorization":f"Bearer {token}"},ping_interval=15,ping_timeout=30):
        try:
            ...
            # BUG12: refresh token before reconnecting
...
async def on_trade(args,rth=True):
    ...
    with LOCK:
        ...
        bar_idx = len(sess.bars) + (1 if sess.cur_bar else 0)
        snap_price = live_last; snap_vwap = sess.vwap; snap_vpoc = sess.vpoc
        snap_std = sess.vwap_std
        snap_bid = live_bid; snap_ask = live_ask  # capture bid/ask for fill pricing

    async with BOT_LOCK:
        await global_bot.check_logic(snap_price, snap_vwap, snap_vpoc, snap_std, cz, bar_idx, rth, stable, snap_bid, snap_ask)
...
<div id="bot-pl" style="background:#0a0a0a;border:1px solid #1a1a1a;margin-bottom:8px;padding:6px;font-size:14px;display:flex;gap:16px;flex-wrap:wrap;">
...
function tickC(tid,pid,tape,prof,H,bars,quotes){
  ...
  if(quotes&&quotes.length)ap.push(...quotes.map(q=>q.bid).filter(v=>v>0),...quotes.map(q=>q.ask).filter(v=>v>0));
  ...
  // Draw bid/ask lines from quotes
  if(quotes&&quotes.length>=2){
    const qsx=cW/Math.max(quotes.length-1,1);
    // Bid line (green)
    x.beginPath();x.strokeStyle='rgba(0,187,102,0.4)';x.lineWidth=1;
    quotes.forEach((q,i)=>{const px=pad.l+i*qsx;if(q.bid>0){if(!i)x.moveTo(px,tY(q.bid));else x.lineTo(px,tY(q.bid));}});
    x.stroke();
    // Ask line (red)
    x.beginPath();x.strokeStyle='rgba(238,51,51,0.4)';x.lineWidth=1;
    quotes.forEach((q,i)=>{const px=pad.l+i*qsx;if(q.ask>0){if(!i)x.moveTo(px,tY(q.ask));else x.lineTo(px,tY(q.ask));}});
    x.stroke();
    // Spread fill between bid/ask
    x.beginPath();x.fillStyle='rgba(255,255,255,0.03)';
    quotes.forEach((q,i)=>{const px=pad.l+i*qsx;if(q.bid>0&&q.ask>0){if(!i)x.moveTo(px,tY(q.ask));else x.lineTo(px,tY(q.ask));}});
    for(let i=quotes.length-1;i>=0;i--){const q=quotes[i],px=pad.l+i*qsx;if(q.bid>0)x.lineTo(px,tY(q.bid));}
    x.closePath();x.fill();
  }
...
async function go(){try{
  ...
  tickC('tc','vp',tp,prof,240,bars,d.quotes||[]);
...
class H2(BaseHTTPRequestHandler):
    def do_GET(self):
        if self.path=="/api/data":
            with LOCK:
                ...
                d["bot"] = {"account_name": global_bot.account_name, "total_pnl": global_bot.total_pnl, "daily_pnl": global_bot.daily_pnl, "trades": global_bot.trades_today, "max_trades": MAX_TRADES, "pos": global_bot.pos, "open_pnl": epnl, "max_dd": global_bot.max_dd, "gutter_win": global_bot.gutter_win, "gutter_loss": global_bot.gutter_loss}
                d["bot_trades"] = global_bot.trade_history
            self.jr(d)
    ...
    def do_POST(self):
        if self.path=="/api/toggle_gw":
            with LOCK:  # BUG1: protect bot state
