...
# Hardcoded logic constraints matching live bot
MAX_STOP_TICKS = 24
MIN_STOP_TICKS = 6
TRAIL_ACTIVATE = 20
TRAIL_DISTANCE = 16
...
def evaluate_params(args):
    ...
    # Penalize low transaction counts (overfitting)
    if total_trades < 30:
        return (z_thresh, contracts, max_trades, stop_ratio, 0.0, 0.0, 0, 0, total_trades)

    wr = len(wins) / total_trades if total_trades else 0
    avg_w = sum(wins) / len(wins) if len(wins) > 0 else 0
    avg_l = abs(sum(losses) / len(losses)) if len(losses) > 0 else 0
    rr = avg_w / avg_l if avg_l > 0 else 0
    expectancy = (wr * avg_w) - ((1 - wr) * avg_l)
    
    # Monte Carlo pass rate (N=500 for sweep speed)
    N_SIMS = 500
    pass_count = 0
    blown_count = 0
    pass_days_list = []
    ...
    # Composite score: pass rate primary, penalise blow rate and slow passes
    speed_bonus = max(0.0, (1.0 - avg_days_to_pass / 45.0)) * 20
    score = (pass_rate * 70) + (expectancy * 0.1) - (blow_rate * 50) + speed_bonus

    return (z_thresh, contracts, max_trades, stop_ratio, pass_rate, expectancy, rr, wr,
            avg_mc_pnl, total_trades, score, blow_rate, avg_days_to_pass)
...
def main():
    ...
    with ProcessPoolExecutor(max_workers=multiprocessing.cpu_count()) as executor:
        for res in executor.map(evaluate_params, grid):
            if res is None or len(res) < 13: continue
            if res[9] < 30: continue  # total_trades < 30 → skip
            results.append({
                "z_thresh":         res[0],
                "contracts":        res[1],
                "max_trades":       res[2],
    ...
    df.to_csv("optimization_results_nq.csv", index=False)
    
    print("\n======= TOP 5 NQ OPTIMIZED PARAMETERS =======")
    cols = ["z_thresh", "contracts", "max_trades", "stop_ratio",
            "pass_rate", "blow_rate", "avg_days_to_pass",
            "win_rate", "risk_reward", "expectancy", "total_trades", "score"]
    print(df[cols].head(5).to_string(index=False))
    print("\nFull data saved to optimization_results_nq.csv for your separate analysis.")
