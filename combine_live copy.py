    ...
    for i in range(1, len(bars)):
        ...
        if pos:
            ...
            if pos["dir"] == "LONG":
                ...
                if pos.get("tp") and h >= pos["tp"] and cur >= EXIT_MIN_TICKS:
                    fill = pos["tp"]
                    ticks = (fill - pos["ep"])/TICK_SIZE; pnl = ticks*TICK_VALUE*_sz; total_pnl += pnl; daily_pnl += pnl
                    trades.append({"action":"EXIT","dir":"LONG","price":fill,"pnl":pnl,"bar_idx":i,"reason":"TARGET","size":_sz})
                    last_exit_bar = i; pos = None; continue
                if ur >= TRAIL_ACTIVATE:
                ...
                early_stop = int(TIME_STOP_MINS * 0.5)
                if bt > TIME_STOP_MINS * 2 or bt > TIME_STOP_MINS or (bt > early_stop and cur < -4):
                ...
                # --- v10 True Mean Exit (full position) ---
                if pos.get("tp") and l <= pos["tp"] and cur >= EXIT_MIN_TICKS:
                    fill = pos["tp"]
                    ticks = (pos["ep"] - fill)/TICK_SIZE; pnl = ticks*TICK_VALUE*_sz; total_pnl += pnl; daily_pnl += pnl
                    trades.append({"action":"EXIT","dir":"SHORT","price":fill,"pnl":pnl,"bar_idx":i,"reason":"TARGET","size":_sz})
                    last_exit_bar = i; pos = None; continue
                if ur >= TRAIL_ACTIVATE:
                    tl = round((pos["bp"] + TRAIL_DISTANCE*TICK_SIZE)/TICK_SIZE)*TICK_SIZE
                    if tl < pos["sl"]: pos["sl"] = tl
            ...
            # Time-of-day filters (mirror live logic)
            if tod_mins >= POWER_HOUR_START: continue
            if LUNCH_SKIP_START <= tod_mins <= LUNCH_SKIP_END: continue
            ...
            # Z-threshold: session-type and crossings heuristics (don't stack).
            bt_crossings = count_vwap_crossings(bt_vwap_history[-20:], [bars[j]["c"] for j in range(max(0,i-20), i+1)], update_regime=False)
            if bt_session_type == "TRENDING":
                bt_effective_z = Z_THRESH * SESSION_TREND_Z_MULT
            elif bt_crossings <= VWAP_CROSS_TRENDING:
                bt_effective_z = Z_THRESH * 1.3
            else:
                bt_effective_z = Z_THRESH
...
def get_combined_exit_trades(gutter_win=True, gutter_loss=True, min_refresh_sec=60):
    now_ts = time.time()
